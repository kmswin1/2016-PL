...
aexp =fun (aexp,x) -> aexp let rec diff (aexp, string) = match aexp with Const n -> Const 0|Var x -> if x=string then Const 1 else aexp|Power (x, n) ->
if x=string then Times [Const n; Power (x, n-1)] else aexp|Times l -> (match l with [] -> Const 0|hd::tl -> Sum([Times(diff(hd, string)::tl)]@[Times(hd::[diff(Times(tl), string)])]))|Sum l -> (match l with [] -> Const 0|hd::tl -> Sum([diff (hd, string)]@[diff (Sum (tl), string)]));;
...
num|INT (n)-> n|ADD (exp1, exp2) -> (cal (exp1, num)) + (cal (exp2, num))|SUB (exp1, exp2) -> (cal (exp1, num)) - (cal (exp2, num))|MUL (exp1, exp2) -> (cal (exp1, num)) * (cal (exp2, num))|DIV (exp1, exp2) -> (cal (exp1, num)) / (cal (exp2, num))|SIGMA (exp1, exp2, exp3) -> (match exp1 with INT (m) -> if exp1=exp2 then cal (exp3, m) else cal (exp3, m)+cal (SIGMA (INT (m+1), exp2, exp3), m));;
(* exception Problem *) 
